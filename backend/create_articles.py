...
# Get or create the category for articles
...
# Get the three featured women
try:
    winnie = User.objects.get(email='winnie.adoma@godlywomen.com')
except User.DoesNotExist:
    print("Winnie not found, creating...")
    winnie = User.objects.create(email='winnie.adoma@godlywomen.com', name='Winnie Adoma')

try:
    hellen = User.objects.get(email='hellen.ojwang@godlywomen.com')
except User.DoesNotExist:
    print("Hellen not found, creating...")
    hellen = User.objects.create(email='hellen.ojwang@godlywomen.com', name='Hellen Ojwang')

try:
    harriet = User.objects.get(email='harriet.madam@godlywomen.com')
except User.DoesNotExist:
    print("Harriet not found, creating...")
    harriet = User.objects.create(email='harriet.madam@godlywomen.com', name='Madam Harriet')

# Create articles for each featured woman
articles_data = [
    {
...
'''
    },
    {
        ...
        'content': '''
<h2>An Accomplished Professional</h2>
